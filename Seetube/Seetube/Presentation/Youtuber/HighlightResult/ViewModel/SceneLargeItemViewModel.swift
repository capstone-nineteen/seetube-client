...
class SceneLargeItemViewModel {
    ...
    convenience init(with scene: HighlightScene) {
        let interval = StringFormattingHelper.toTimeIntervalFormatString(startSecond: Int(scene.startTimeInOriginalVideo),
                                                                         endSecond: Int(scene.endTimeInOriginalVideo))
